...
dir.create(paste0(water_yr_out_path,'cpts/'),showWarnings = F,recursive = T)
...
get_flow_cpts <- function(x,out_path,sig_yr_path,Sigs_selected,yearType){
  source('H:/0001Work/Global_analyze_detect/Code/Global_sigs_analysis/04_cpts/functions/get_cpts.R')
  # x <- dt_annual[Name %in% 'AUS-HRS_215207',]
  print(unique(x$Name))
  ...
  # methods_cpt <- c('mcp1')
  methods_cpt <- c('Bayesian','AMOC','PELT',"SegNeigh" , "BinSeg",
                   'RBeast','cpm','Cusum','mk', 'EnvCpt',
                   'segmented','mcp1','Strucchange','Kmeans', 'mcp2')
  Sigs_selected0 <- intersect(names(TS_sig),Sigs_selected)
  
  out_all <- lapply(as.list(methods_cpt ), 
         function(cptMethod,TS_sig, Sigs_selected){
          out <-  get_cpts(TS_sig, Sigs_selected, NAthresh = 0.95,
                           cptMethod , if_fill = T)
  ...
  out_all %<>% mutate(STAID = unique(x$STAID),
                      yearType = yearType,
                      cpt_filtered2 = floor(cpt_filtered) )
  out_all %<>% setDT()
  fwrite(out_all,paste0(out_path,'cpts/',unique(x$Name),'.csv'))
  
}



done2 <- list.files(paste0(out_path,'cpts/'),pattern = '.csv',full.names = F) %>% 
  str_remove(.,'.csv') %>%
  data.table(Name = .)
# # 
library(parallel)
ncores <- detectCores(logical=F) # physical cores
ncores <- min(8,ncores-1)
cl <- parallel::makeCluster(ncores)

parLapply(cl,
# lapply(
          split(dt_annual[!Name %in% done2$Name,],dt_annual[!Name %in% done2$Name,]$STAID),
          function(x,out_path,water_yr_out_path,sig_yr_path,water_yr_path,Sigs_selected, get_flow_cpts ){
