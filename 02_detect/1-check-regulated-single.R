...
# Not Influenced by dams
# Attr_filtered <- info_new[(GRanD < 10 | is.na(GRanD)) & 
#                          (GDAT < 10 | is.na(GDAT)) &
#                          (dor_pc_pva < 100 | is.na(dor_pc_pva)),]
...
out_path <- 'F:/0001Work/02_Globalstreamflow_update/05_Runoff_Signatures_update/Regulated/SingleDam/'
...
get_normalized_data <- function(iseq,Attr_filtered2,signature_path,flow_path,out_path,Sigs){
  ...
  # normalized_Data
  data_flow %<>% mutate(.,
                        Norm_Index = Year - info_n$Dams_year_Max,
  ...
  data_sig <- data %>% melt(.,
    ...
    summarise(.,
              ...
              ncount = ncount,
              Index = Index,
              Norm_Index = Index - info_n$Dams_year_Max,
              Value = Value,
  ...
  data_sig_plot <- data_sig[ncount >= 365 & 
                              Signatures %in% Sigs,]
...
# Sigs <- c("Index",
#           "Quantile_99","Quantile_50","Quantile_01",
#           "HFI","HFI_Q90","HFI_Q99",
...
#           "MHFD", "MLFD", "MZFD",
#           "MHFD_Q90", "MLFD_Q10", "ZFI",
#           "HFT","QT1","QT3",
#           "GINI" ,"GINI_Acc","FDCs","CI","cv","SD","SD_log")
# Sigs <- c("Quantile_99","Max_peak","Mean_peak_Q99","HFI_Q90","MHFD_Q90",
#           "Quantile_01","ZFI","Mean_low_Q10","LFI_Q10","MZFD"
#           
# )
